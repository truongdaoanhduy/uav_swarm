"""
MATD3 Trainer — Kaggle optimized (no viz/plot during training)
...
class _EnvWrapper:
    ...
    def step(self, actions_batch: np.ndarray):
        ...
        obs_t = np.array(
            [obs_d.get(f"uav_{i}", np.zeros(self.obs_dim, np.float32)) for i in range(n)],
            dtype=np.float32,
        )[None]
        g_t  = info["uav_0"]["global_obs"][None]
        rews = np.array(
            [rew_d.get(f"uav_{i}", 0.0) for i in range(n)],
            dtype=np.float32,
        )[None]
        ...
        if done:
            ...
            new_obs = np.array(
                [new_obs_d.get(f"uav_{i}", np.zeros(self.obs_dim, np.float32)) for i in range(n)],
                dtype=np.float32,
            )[None]
            self._current_obs    = new_obs
            self._current_global = new_info["uav_0"]["global_obs"][None]
        else:
            self._current_obs    = obs_t
            self._current_global = g_t

        return obs_t, g_t, rews, [done], [info]
...
class MATD3Trainer:
    ...
    def __init__(
        ...
        self.config   = config
        self.n_envs   = n_envs
        self.run_name = run_name or f"matd3_{int(time.time())}"
        ...
        self.buffer_capacity  = tr.matd3_buffer_capacity
        self.batch_size       = tr.matd3_batch_size
        self.gamma            = tr.matd3_gamma
        self.tau              = tr.matd3_tau
        self.policy_delay     = tr.matd3_policy_delay
        self.explore_noise    = tr.matd3_explore_noise
        self.target_noise     = tr.matd3_target_noise
        self.noise_clip       = tr.matd3_noise_clip
        ...
        self.critic = TD3TwinCriticNetwork(
            global_obs_dim = self.global_obs_dim,
            action_dim     = self.action_dim,
            hidden_dims    = tr.matd3_critic_hidden,
        ).to(self.device)

        self.critic_target = TD3TwinCriticNetwork(
            global_obs_dim = self.global_obs_dim,
            action_dim     = self.action_dim,
            hidden_dims    = tr.matd3_critic_hidden,
        ).to(self.device)
        ...
        # ── 5. Optimizers ─────────────────────────────────────────────────
        self.actor_opt  = torch.optim.Adam(self.actor.parameters(),  lr=tr.matd3_lr_actor)
        self.critic_opt = torch.optim.Adam(self.critic.parameters(), lr=tr.matd3_lr_critic)
        ...
        self.output_dir     = base_dir / "matd3" / self.run_name
        ...
        while self.total_episodes_done < total_episodes:
            ...
            with torch.no_grad():
                if self.total_steps < self.warmup_steps:
                    ...
                    act_np[:, 3] = np.random.randint(0, 2, size=(n * self.n_agents,)).astype(np.float32)
                else:
                    act_t, _ = self.actor.get_action(
                        obs_t,
    ...
    def _update(self) -> Dict:
        ...
        # ── 1. TD target ──────────────────────────────────────────────────
        with torch.no_grad():
            next_act_flat = self.actor_target.get_target_action(
                next_obs_flat,
                target_noise = self.target_noise,
                noise_clip   = self.noise_clip,
            )
            next_act_mean = next_act_flat.reshape(B, A, self.action_dim).mean(dim=1)
            q_next        = self.critic_target.min_q(next_g, next_act_mean).squeeze(-1)
            target        = team_reward + self.gamma * (1.0 - dones) * q_next
        ...
        if self._critic_update_count % self.policy_delay == 0:
            ...
            act_flat_new = self.actor.forward(obs_flat)
            act_new_mean = act_flat_new.reshape(B, A, self.action_dim).mean(dim=1)

            actor_loss_t = -self.critic.q1_value(g_obs, act_new_mean).mean()
    ...
    def save_checkpoint(self, episode: int, curriculum_manager=None, tag: str = None):
        ...
        if self.ep_rewards:
            print(
                f"   ep={episode:,} | rew={np.mean(self.ep_rewards):+.1f} | "
    ...
    def load_checkpoint(self, path: str) -> int:
        ...
        self._critic_update_count = ckpt.get("critic_update_count", 0)
        ...
        print(f"✅ MATD3 loaded: {path} | ep={self.total_episodes_done:,}")
    ...
    def _log_detail(self, pbar, metrics: Dict, elapsed: float):
        lines = [
            f"\n{'─'*65}",
            f"⚡ MATD3 | ep={self.total_episodes_done:,} | upd={self.update_count:,} | {elapsed/60:.1f}min",
            f"{'─'*65}",
            f"  Task:",
        ...
        if metrics:
            lines += [
                ...
                f"    delay_cnt: {self._critic_update_count:>10,}",
    ...
    def _print_init(self):
        print(f"\n{'='*60}")
        print(f"⚡ MATD3 Trainer — Kaggle Mode")
        print(f"{'='*60}")
        ...
        print(f"  policy_delay : {self.policy_delay}")
        print(f"  noise        : explore={self.explore_noise}, target={self.target_noise}")
        print(f"  actor_hidden : {self.config.train.matd3_actor_hidden}")
        print(f"  critic_hidden: {self.config.train.matd3_critic_hidden}")
        print(f"  hf_upload    : {self.hf_uploader is not None}")
    ...
    def _print_final(self, elapsed: float):
        print(f"\n{'='*60}")
        print(f"✅ MATD3 Done | {elapsed/60:.1f} min ({elapsed/3600:.2f}h)")
