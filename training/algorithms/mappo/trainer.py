"""
MAPPO Trainer - SIMPLE LOGGING
✅ tqdm(range(...)) auto-update
✅ Manual print mỗi N episodes
✅ Không dùng pbar.write()
"""
...
class MAPPOTrainer:
    ...
    def __init__(
        ...
        print(f"  Episode log: every {self.log_every}")
    ...
    def get_values(self, global_obs):
    ...
    def train(self, total_episodes, curriculum_manager=None, seed=42):
        ...
        # ✅ SIMPLE: tqdm bọc range
        with tqdm(total=total_episodes, desc="Training", unit="ep") as pbar:
            while self.total_episodes_done < total_episodes:
                # Rollout
                rollout_metrics = self.rollout(env, max_episodes=total_episodes)
                ...
                pbar.n = self.total_episodes_done
                pbar.refresh()
                ...
                if update_count % self.log_interval == 0:
                    ...
                    print(f"\n{'─'*60}")
                    print(f"📊 Update {update_count} | Episodes {self.total_episodes_done}/{total_episodes}")
                    print(f"{'─'*60}")
                    print(f"  Task   | Reward: {rollout_metrics['mean_ep_reward']:+8.2f} | "
                          f"Coverage: {rollout_metrics['mean_coverage']:5.1f}% | "
                          f"Victims: {rollout_metrics['mean_victims']:5.1f}%")
                    print(f"  Train  | Actor: {train_metrics['actor_loss']:7.4f} | "
                          f"Critic: {train_metrics['critic_loss']:7.4f} | "
                          f"Entropy: {train_metrics['entropy']:6.4f}")
                    print(f"  Perf   | FPS: {fps:6.1f} | Steps: {self.total_steps_collected:,} | "
    ...
    def rollout(self, env, max_episodes=None):
        if self.n_envs == 1:
            return self._rollout_single(env, max_episodes)
        else:
            return self._rollout_vectorized(env, max_episodes)
    
    def _rollout_single(self, env, max_episodes=None):
        ...
        while steps_collected < self.rollout_length:
            if max_episodes and self.total_episodes_done >= max_episodes:
                break
            
            actions_dict, actions_np, log_probs_np = self.select_action(obs_dict)
            ...
            obs_np = np.array([obs_dict[aid] for aid in agent_ids], dtype=np.float32)
            self.buffer.add(obs_np, global_obs, actions_np, rewards_np, values_np, log_probs_np, done)
        ...
        self.buffer.compute_gae(last_values, done)
    ...
    def _rollout_vectorized(self, env, max_episodes=None):
        ...
        for step_idx in range(self.rollout_length):
            ...
            obs_flat = obs_batch.reshape(self.n_envs * self.n_agents, self.obs_dim)
            obs_tensor = torch.FloatTensor(obs_flat).to(self.device)
            
            with torch.no_grad():
                actions_tensor, log_probs_tensor = self.actor.get_action(obs_tensor)
            
            actions_batch = actions_tensor.cpu().numpy().reshape(self.n_envs, self.n_agents, self.action_dim)
            log_probs_batch = log_probs_tensor.cpu().numpy().reshape(self.n_envs, self.n_agents)
            
            global_obs_tensor = torch.FloatTensor(global_obs_batch).to(self.device)
            with torch.no_grad():
                values_per_env = self.critic.get_value(global_obs_tensor).cpu().numpy()
            
            values_batch = np.stack([np.full(self.n_agents, v, dtype=np.float32) for v in values_per_env])
            
            next_obs_batch, next_global_obs_batch, rewards_batch, dones, infos = env.step(actions_batch)
            
            for env_idx in range(self.n_envs):
                self.buffer.add(
                    obs_batch[env_idx],
                    global_obs_batch[env_idx],
                    actions_batch[env_idx],
                    rewards_batch[env_idx],
                    values_batch[env_idx],
                    log_probs_batch[env_idx],
                    dones[env_idx]
                )
                ...
                if dones[env_idx]:
                    ...
                    if 'uav_0' in info_e:
                        ...
                        if self.total_episodes_done % self.log_every == 0:
                            ...
                            vic_rate = vic_found / vic_total * 100
                            print(f"{icon} Ep {self.total_episodes_done:>5d} | "
        ...
        with torch.no_grad():
            last_values_per_env = self.critic.get_value(
                torch.FloatTensor(global_obs_batch).to(self.device)
            ).cpu().numpy()
        
        mean_last_value = float(np.mean(last_values_per_env))
        last_values = np.full(self.n_agents, mean_last_value, dtype=np.float32)
        self.buffer.compute_gae(last_values, False)
    ...
    def update(self):
        actor_losses, critic_losses, entropies, clip_fractions = [], [], [], []
    ...
    def _save_visualization(self, env, update):
    ...
    def save_checkpoint(self, update, curriculum_manager=None):
        checkpoint = {
            'update': update,
